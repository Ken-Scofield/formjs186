...
export function formatTime(use24h, minutes) {
  ...
  if (use24h) {
    return _getZeroPaddedString(hour) + ':' + _getZeroPaddedString(minute) + ':00'; // :kim
  }
  ...
  return _getZeroPaddedString(hour) + ':' + _getZeroPaddedString(minute) + ':00 ' + (isPM ? 'PM' : 'AM');
}
...
export function isDateTimeInputInformationSufficient(value) {
  ...
  const segments = value.split('T'); // kim: const segments = value.split('T');
...
export function isDateInputInformationMatching(value) {
  ...
  if (value.includes('T')) return false; // kim:
